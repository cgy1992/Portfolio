...
void SWGE::Graphics::Texture::Initialize(DepthMap & depthMap)
{
	ASSERT(depthMap.mShaderResourceView != nullptr, "RenderTarget was not Initialized");
	// Check if shaderResourceView is null 
	if (mShaderResourceView)
	{
		SafeRelease(mShaderResourceView);
	}
	mShaderResourceView = depthMap.mShaderResourceView;
	//Add reference count because both are expected to terminate
	mShaderResourceView->AddRef();
}

void SWGE::Graphics::Texture::Initialize(RenderTarget & renderTarget)
{
	ASSERT(renderTarget.mShaderResourceView != nullptr, "RenderTarget was not Initialized");
	if (mShaderResourceView)
	{
		SafeRelease(mShaderResourceView);
	}
	mShaderResourceView = renderTarget.mShaderResourceView;
	mShaderResourceView->AddRef();
}
...
void SWGE::Graphics::Texture::UnbindVS(uint32_t slot) const
{
...
void SWGE::Graphics::Texture::UnbindPS(uint32_t slot) const
{
	static ID3D11ShaderResourceView* dummy = nullptr;
